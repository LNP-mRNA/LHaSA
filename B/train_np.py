...
def main(args) -> None:
    ...
    # HNM: mixed_sampler will be initialized after train dataset is loaded
    mixed_sampler = None
    ...
    # ===================== Hard Negative Mining: 初始化 MixedSampler =====================
    # 必须在 checkpoint_utils.load_checkpoint 之后初始化，此时 train dataset 已加载
    hnm_enabled = getattr(args, 'hard_negative_mining', False)
    if hnm_enabled and mixed_sampler is None:
        try:
    ...
    while epoch_itr.next_epoch_idx <= max_epoch and epoch_itr.next_epoch_idx <= epoch_to_stop:
        ...
        # ===================== Hard Negative Mining: 定期挖掘 =====================
        if hnm_enabled and mixed_sampler is not None:
    ...
    # ===== Training finished: evaluate on TEST set =====
    if distributed_utils.is_master(args):
        test_subsets = ["test"]
        # Load test dataset
...
def train(
    ...
    use_hnm = getattr(args, 'hard_negative_mining', False) and mixed_sampler is not None
...
def validate(
    ...
    with utils.torch_seed(seed):
        ...
        for subset in subsets:
            ...
            # Determine if validation or test set, display different title
            is_test = (subset == "test")
            panel_icon = "TEST" if is_test else "VALIDATION"
